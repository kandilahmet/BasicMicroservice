...
namespace OrderService.Core.Application.Features.Commands.Create
{
    public class CreateOrderCommandHandler : IRequestHandler<CreateOrderCommandRequest, bool>
    {
        ...
        public CreateOrderCommandHandler(IEFCoreWriteRepository<Order> writeOrderRepository, IPublishEndpoint publisher)
        {
        ...
        public async Task<bool> Handle(CreateOrderCommandRequest request, CancellationToken cancellationToken)
        {
            ...
            var order = new Order()
            {
                ...
                OrderItems = request.CreateOrderItems.Select(oi => new OrderItem()
                {
                    Count = oi.Count,
                    Price = oi.Price,
                    ID = Guid.NewGuid(),
            ...
            if (result)
            {
                var orderCreatedEvent = new OrderCreatedEvent()
                {
                    ...
                    OrderId = order.ID,
                    OrderItems =
                        order.OrderItems.Select(oi => new OrderItemMessage()
                        {
                            Count = oi.Count,
                            ProductId = Convert.ToString(oi.ProductId)
                        }).ToList()
                    ,
